...
export default function CommunityItem({
  ...
  return (
    <div
      className={cn(
        'h-[160px] flex p-[20px] box-border items-center gap-[20px] bg-[#1B1E23] rounded-[20px]',
        className
      )}
      {...props}
    >
      <img src={logo} alt="" className="w-[120px] h-[120px] rounded-[20px]" />
      <div className="flex-1 flex h-full flex-col justify-between items-start">
        <div className="w-full flex justify-between items-start gap-[10px]">
          <div className="flex-1 flex flex-col justify-between items-start gap-[10px]">
            {types?.length > 0 && (
              <div className="flex-1 text-[#718096] text-[12px] font-normal line-clamp-1">
        ...
        <div className="text-[#FFF] text-[14px] font-normal leading-[20px] mt-[20px] line-clamp-2">
          {description}
        </div>
      </div>
    </div>
