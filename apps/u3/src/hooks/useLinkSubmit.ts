...
export default function useLinkSubmit() {
  ...
  const createDappLink = useCallback(
    async (url: string, linkData: DappLinkData) => {
      if (!url) return;
      try {
        validateS3LinkModelPermission();
        const link = await s3linkModel.createLink({
  ...
  const updateDappLinkData = useCallback(
    async (linkId: string, linkData: Partial<DappLinkData>) => {
      if (!linkId) return;
      try {
        validateS3LinkModelPermission();
        const res = await updateLinkData(linkId, linkData);
